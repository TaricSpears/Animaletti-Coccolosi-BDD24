...
public class PopulateTherapyWithWorkoutButton extends Button {
    public PopulateTherapyWithWorkoutButton(final String email, final Stage primaryStage,
            final Tab previousTab) {
        this.setText("Popola terapia con esercizio settimanale");
    ...
    private boolean isValid(final String idTerapia, final String email) {
        ...
        String query4 = "SELECT * FROM prescrizione_e WHERE Codice_Terapia = ?";
        try (Connection connection = MySQLConnect.getConnection();
            ...
            if (resultSet2.next()) {
                ...
                alert.setTitle("Errore");
                alert.setHeaderText("Errore");
                alert.setContentText("La terapia e' gia' associata ad un workout");
                alert.showAndWait();
                return false;
            }
        } catch (SQLException e) {
